        ...
        switch ($mode) {
            case 'selected':
                if (Mage::getSingleton('admin/session')->isAllowed('admin/sales/order/actions/view'))
                    $retour = '<a href="' . $this->getUrl('adminhtml/sales_order/view', array('order_id' => $row->getorder_id())) . '" target="_blank">' . $this->__('View order') . ' </a>';
            ...
            case 'fullstock':
                if (Mage::getSingleton('admin/session')->isAllowed('admin/sales/order/actions/view'))
                    $retour = '<a href="' . $this->getUrl('adminhtml/sales_order/view', array('order_id' => $row->getopp_order_id())) . '" target="_blank">' . $this->__('View order') . ' </a>';
                if (Mage::getSingleton('admin/session')->isAllowed('admin/erp/order_preparation/prepare_order/mass_actions/add_to_selection'))
