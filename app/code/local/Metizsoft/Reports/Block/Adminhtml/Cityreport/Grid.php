...
class Metizsoft_Reports_Block_Adminhtml_Cityreport_Grid extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    protected function _prepareColumns() {
        ...
        $this->addColumn('order_qty', array(
            'header' => Mage::helper('metizsoft_reports')->__('County'),
            'index' => 'county',
            'width' => 200
        ));
        $this->addColumn('city', array(
            'header' => Mage::helper('metizsoft_reports')->__('City'),
            'index' => 'city',
            'width' => 200
        ));
        ...
        // add export types
        $this->addExportType('*/*/exportcityCsv', Mage::helper('metizsoft_reports')->__('CSV'));
        $this->addExportType('*/*/exportcityExcel', Mage::helper('metizsoft_reports')->__('MS Excel XML'));
    ...
    protected function _prepareCollection() {
        ...
        if ($county != ''){
            $resourceCollection->addFieldToFilter('county', $county);
        }
        //$city = Mage::getModel('metizsoft_taxgenerate/countylist')->load($data['city']);
        if ($city != ''){
            $resourceCollection->addFieldToFilter('city', $city);
        }
        
        $resourceCollection->addFieldToFilter('created', array('gteq' => $from));
        $resourceCollection->addFieldToFilter('created', array('lteq' => $to));
        
        $resourceCollection->getSelect()->group('city');
        $resourceCollection->getSelect()
                ->columns('SUM(citytax) as totalstatetax, SUM(protaxunit) as protaxunit')
                ->group('catname');
        $resourceCollection->getSelect()->having('SUM(citytax) > 0');
        
        //echo '<pre>';print_r($resourceCollection->getData());exit;
