...
class Metizsoft_Reports_Block_Adminhtml_Countyreport_Grid extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    protected function _prepareColumns() {
        ...
        // add export types
        $this->addExportType('*/*/exportcountyCsv', Mage::helper('metizsoft_reports')->__('CSV'));
        $this->addExportType('*/*/exportcountyExcel', Mage::helper('metizsoft_reports')->__('MS Excel XML'));
    ...
    protected function _prepareCollection() {
        ...
        $county = Mage::getModel('metizsoft_taxgenerate/countylist')->load($data['county']);
        
        if ($county->getName()){
            $resourceCollection->addFieldToFilter('county', $county->getName());
        }
        
        $resourceCollection->addFieldToFilter('created', array('gteq' => $from));
        $resourceCollection->addFieldToFilter('created', array('lteq' => $to));
        
        $resourceCollection->getSelect()->group('county');
        $resourceCollection->getSelect()
                ->columns('SUM(countytax) as totalstatetax, SUM(protaxunit) as protaxunit')
                ->group('catname')
                ->having('SUM(countytax) > 0');
