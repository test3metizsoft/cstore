...
class Infomodus_Upslabel_Adminhtml_UpslabelController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function intermediatehandy($order_id, $type, $shipment_id = NULL)
    {
        ...
        foreach ($shipmentAllItems AS $item) {
            ...
            if(!isset($itemData['qty'])){$itemData['qty'] = $itemData['qty_ordered'];}
        ...
        $this->shipByUps = preg_replace("/^ups_.{1,6}$/", 'ups', $ship_method);
        $this->shipByUpsCode = $this->configMethod->getUpsMethodNumber(preg_replace("/^ups_(.{1,6})$/", '$1', $ship_method));
        ...
        } else if (Mage::getStoreConfig('upslabel/shipping/shipping_method_native') == 1 && $modelConformity && count($modelConformity) > 0) {
        ...
        $this->defConfParams['cod'] = Mage::getStoreConfig('upslabel/ratepayment/cod');
    ...
    public function setDefParams($itemData = NULL , $type = "shipment")
    {
        ...
        $defParArr_1['cod'] = Mage::getStoreConfig('upslabel/ratepayment/cod');
